...
#ifdef OMP
#pragma omp critical // seems faster to read sequentially
#endif
    { 
...
//  number_of_iterations = std::max(MIN_THREAD_COUNT, number_of_iterations);
//
//  //Image<uint16_t> frame_stack( input->frame_height, input->frame_width, input->number_of_frames ); 
////  Image<uint16_t> frame_stack( 16048, 256, 256 ); 
//  //frame_stack.fillZeros();
//  //Image<uint16_t> *frame_stack = new Image<uint16_t>( input->frame_height, input->frame_width, input->number_of_frames ); 
//  //Image<uint16_t> *frame_stack = new Image<uint16_t>( input->frame_height, input->frame_width, MAX_FRAMES_PER_ITERATION ); 
//  //Image<uint16_t> *frame_stack = new Image<uint16_t>( 5, 5, 2 ); 
//  //std::cout << *frame_stack;
//
//#ifdef OMP
//  omp_set_num_threads(1);
//  //omp_set_num_threads(number_of_iterations);
...
//
//      frame_stack = new Image<uint16_t>( total_num_frames, input->frame_height, input->frame_width ); 
//      //frame_stack->fillZeros();
//
//      ///// Step 0: Raw Data Acquisition /////
//      input->parseImageData( start, end, data_set );
//      input->parseImageData( start, end, frame_stack );
//
//      // compare both data structures
////      for ( int qq = 0; qq < total_num_frames; qq++ ) {
////        for ( int ww = 0; ww < 256; ww++ ) {
////          for ( int ee = 0; ee < 256; ee++ ) {
////            if ( (uint16_t)data_set[qq][ww][ee] != (uint16_t)(*frame_stack)[std::make_tuple(qq, ww, ee)] ) {
////              printf("frame: #%d, pixel: %dx%d\n", qq, ww, ee);
////              //printf("%d -- %d\n", (uint16_t)data_set[qq][ww][ee], (uint16_t)(*frame_stack)[std::make_tuple(qq, ww, ee)]);
////              printf("FAIL\n");
////              exit(1);
////            }
////          }
...
//}
//
//void ComputeManager::compute2(char* inversegain_path, char* varmap_path, char* data_path, char* output_path, 
//                              int min_photon, float PSFSigma, int zm_color, float SRImage_pixelsize, int ll_threshold, 
//                              int subregion_cropx, int subregion_cropy, int N_photon_min, int N_photon_max, float loc_uncer_min, float loc_uncer_max, 
